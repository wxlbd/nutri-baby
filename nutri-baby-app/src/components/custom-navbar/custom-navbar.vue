<template>
    ...
    >
        <view
            ...
            :style="{ height: navbarContentHeight + 'rpx' }"
        >
            <!-- 左侧宝宝信息 -->
...
const babyAge = computed(() => {
    console.log("currentBaby.value", currentBaby.value);
...
onMounted(() => {
    ...
    try {
        ...
        if (menuButton) {
            ...
            const contentHeight =
                (menuButtonTop - statusBarHeight.value) * 2 + menuButtonHeight;
            // 转换为 rpx (px * 2)
...
// 导出导航栏总高度供父组件使用
defineExpose({
    navbarTotalHeight: computed(
        () => statusBarHeight.value * 2 + navbarContentHeight.value,
    ),
});
</script>
