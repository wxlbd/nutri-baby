        ...
        <view class="form-wrapper">
            ...
            <view v-if="feedingType === 'bottle'" class="form-section">
                ...
                <view class="form-row">
                    ...
                    <view class="form-group">
                        ...
                        <view class="input-group">
                            <button class="input-btn" @click="bottleForm.amount = Math.max(0, bottleForm.amount - 10)">−</button>
                            <text class="input-value">{{ bottleForm.amount }}</text>
                            <button class="input-btn" @click="bottleForm.amount = Math.min(500, bottleForm.amount + 10)">+</button>
                        </view>
                    </view>
                </view>
                ...
                <view v-if="bottleForm.amount > 0" class="form-group">
                    ...
                    <view class="input-group">
                        <button class="input-btn" @click="bottleForm.remaining = Math.max(0, bottleForm.remaining - 5)">−</button>
                        <text class="input-value">{{ bottleForm.remaining }}</text>
                        <button class="input-btn" @click="bottleForm.remaining = Math.min(bottleForm.amount, bottleForm.remaining + 5)">+</button>
                    </view>
                </view>
            </view>
                ...
                <view v-if="reminderEnabled" class="reminder-settings">
                    ...
                    <view class="reminder-interval">
                        ...
                        <view class="custom-interval">
                            ...
                            <view class="input-group">
                                <button class="input-btn" @click="reminderInterval = Math.max(1, reminderInterval - 15)">−</button>
                                <text class="input-value">{{ reminderInterval }}</text>
                                <button class="input-btn" @click="reminderInterval = Math.min(1440, reminderInterval + 15)">+</button>
                            </view>
                        </view>
                    </view>
                </view>
...
// 喂养类型选项
const feedingTypes: Array<{ label: string; value: 'breast' | 'bottle' | 'food' }> = [
...
// 监听喂养类型变化,加载对应的提醒偏好
watch(() => feedingType.value, () => {
  loadReminderPreferences()
  console.log('[Feeding] 喂养类型已变更,提醒间隔已更新:', reminderInterval.value)
})
...
const loadReminderPreferences = () => {
  ...
  if (prefs && prefs[feedingType.value]) {
    ...
    console.log('[Feeding] 已加载用户偏好 - 喂养类型:', feedingType.value, '间隔:', reminderInterval.value)
  } else {
    // 使用默认值
    const defaults = { breast: 180, bottle: 180, food: 240 }
    reminderInterval.value = defaults[feedingType.value as 'breast' | 'bottle' | 'food'] || 180
...
const saveReminderPreference = () => {
  const prefs = getStorage<any>(StorageKeys.FEEDING_REMINDER_PREFERENCES) || { breast: 180, bottle: 180, food: 240 }
  prefs[feedingType.value] = reminderInterval.value
  setStorage(StorageKeys.FEEDING_REMINDER_PREFERENCES, prefs)
  console.log('[Feeding] 已保存用户偏好 - 喂养类型:', feedingType.value, '间隔:', reminderInterval.value)
}
